...
public class CompilationProcess {
  ...
  private final CreateDatabaseQueries createDatabaseQueries; // 1.4. TRANSPILER: Based on the logicalPlan (calcite schema) keep track of the APIQueries (wrapper that links together database query parameters, graphQL query and corresponding Calcite relNode in the logical plan) for the table functions. This is needed when we don't have an API for a sqrl script. In that case, we cannot infer the queries from the API schema and instead generate them from the script defined tables.
